...
class WeatherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("IoT Weather Station")
    ...
    def update_data(self):
        while self.running:
            ...
            if data:
                self.temp_label.config(text="Temperature: {} °C".format(data['temperature']))
                self.hum_label.config(text="Humidity: {} %".format(data['humidity']))
                send_command("PING")
            time.sleep(2)
